...
> **サンプル音源を使う場合**: ユーザーが自前の音源を持っていないときは、サンプルパックを案内できます。インストール済みの vox-radio と同じバージョンのパックを取得・展開してもらいます（手順 1・2 は省略可）。
>
> ```bash
...
> ```
>
> これで設定ファイルと同じ場所に `assets/assets.yaml`（音源登録済み）が用意される。手順 3（鳴らし方の紐付け）から進める。音源の提供元・ライセンスは展開後の `assets/CREDITS.md` を参照し、ユーザーに利用規約（OtoLogic 素材はクレジット表記が必要）を案内すること。
