---
name: vox-radio
description: vox-radio でラジオ番組を作る・更新する／設定ファイル（vox-radio.yaml・episode-spec.yaml・アセット設定・feed-spec.yaml・slack-spec.yaml）を編集する／設定や使い方を相談する／vox-radio 本体を最新版へ更新するスキル。質問に答えるだけで番組（mp3）まで仕上げられる。
...
> **vox-radio 本体を最新版へ更新したいとき**（「最新版にして」「バージョンを上げて」等の依頼や、上記でバイナリが古いと判明したとき）は、`references/update.md` の手順（最新版確認 → バイナリ更新 → スキル再インストール → 設定の追随 → 検証）に従ってください。
...
> **サンプル音源を使う場合**: ユーザーが自前の音源を持っていないときは、サンプルパックを案内できます。インストール済みの vox-radio と同じバージョンのパックを、設定ファイルのあるディレクトリで取得・展開してもらいます（手順 1・2 は省略可）。
>
> ```bash
...
> ```
>
> `assets/assets.yaml`（音源登録済み）が用意される。手順 3（鳴らし方の紐付け）から進める。音源の提供元・ライセンスは展開後の `assets/CREDITS.md` を参照し、ユーザーに利用規約（OtoLogic 素材はクレジット表記が必要）を案内すること。
...
- **生成する**（前提チェックが整っている場合）: 次を実行し `output/{program.id}_ep{NNN}.mp3` を生成する
...
生成物のパス（`output/{program.id}_ep{NNN}.mp3`、マニフェストは `output/{program.id}_ep{NNN}_manifest.json`、中間ファイルは `output/intermediate/{program.id}_ep{NNN}/`）を伝えます。合成音声を公開する場合は VOICEVOX のクレジット表記（例: `VOICEVOX:ずんだもん`）が必要な点も案内してください。
