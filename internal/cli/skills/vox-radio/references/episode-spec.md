...
| `timezone` | string | 任意 | 番組内で日時を整形するときのIANAタイムゾーン名（例: `Asia/Tokyo`）。記事の配信日時・収録日時の整形、および生成 MP3 の録音日時タグ（ID3 TDRC）の整形に使用。未指定時はデフォルト `Asia/Tokyo`。不正なタイムゾーン名は警告のうえ UTC にフォールバックする |
