...
| フィールド | 型 | 必須/任意 | 説明 |
|---|---|---|---|
| `id` | string | 必須 | 番組を識別するID。過去回の記憶（キャッシュ）の保存キー・配信フィードの突き合わせに使用。日替わりコーナーやゲストの登場回もこのIDをキーに数える |
...
- キャッシュ読み込みに失敗して回番号が不明な場合、条件付きコーナーを含む全コーナーが採用されます（警告ログが出力されます）
...
- キャッシュ読み込みに失敗して回番号が不明な場合、条件付きキャスト（ゲスト含む）は出演しません（警告ログが出力されます）
