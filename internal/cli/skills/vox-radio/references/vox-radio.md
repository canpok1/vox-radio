...
- 台本テキストを確定する際、LLM による読み変換マップ（`line_conversions`）を適用する**前段**で、セリフ中に現れた登録表記を読み方へ置換します。同じ表記が複数回現れればすべて置換されます。
- 未登録の語は変換されず、そのまま残ります。
- 登録した表記が部分的に重なる場合は、長い表記を優先して置換します（例: `東京` と `東京駅` を登録すると `東京駅` を優先）。
- 同じ行を LLM がかな化した場合は、辞書置換よりも LLM の結果が優先されます（辞書は LLM が変換しなかった行に効きます）。
