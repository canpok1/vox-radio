...
2. `slack-spec.yaml` の `slack.channel_env` が指す環境変数からチャンネル ID を取得
3. `manifest.json` を読み込み、mp3 パスを自動解決（manifest と同じディレクトリ + `audio_file`）
4. `slack-spec.yaml` を読み込み・検証（テンプレートファイルの存在・構文も検証）
