...
例:
  ...
  vox-radio episodegen script --in work/intermediate/02_rundown.json --out work/intermediate/04_script.json --profile sample-profiles/tech_profile.yaml`,
...
func runScriptWrite(ctx context.Context, in, workDir string, c llm.Client, cfg *config.Config, p *config.Profile, prompts map[string]string) error {
	if in == "" {
