...
例:
  vox-radio episodegen manifest --profile sample-profiles/tech_profile.yaml --audio output/episode.mp3 --out output/manifest.json
  vox-radio episodegen manifest --profile sample-profiles/tech_profile.yaml --rundown output/intermediate/02_rundown.json --audio output/episode.mp3 --out output/manifest.json
  vox-radio episodegen manifest --profile sample-profiles/tech_profile.yaml --lines output/intermediate/03_lines.json --audio output/episode.mp3 --out output/manifest.json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if linesPath != "" {
				...
				ps := programsummary.NewLLMProgramSummarizer(llmClient, prompts["summary"], stepTemp(cfg.LLM, "summary"), p.Program.EffectiveSummaryLength())
				programSummary, err = ps.Summarize(context.Background(), scriptLines)
				if err != nil {
					return fmt.Errorf("summarize program: %w", err)
				}

				cs := programsummary.NewLLMCornerSummarizer(llmClient, prompts["corner_summary"], stepTemp(cfg.LLM, "corner_summary"))
			...
			m := manifest.Build(p.Program, p.Corners, rd, filepath.Base(audioPath), time.Now().UTC(), programSummary.Summary, cornerSummaries, programSummary.ConversationNotes, 0, programSummary.EpisodeTitle)
