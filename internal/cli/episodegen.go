...
例:
  ...
  vox-radio episodegen --out-dir output --spec examples/tech.yaml`,
		RunE: func(cmd *cobra.Command, args []string) error {
			logger, logFile, err := setupLogger("episodegen", "")
			if err != nil {
			...
			selectedGuests := guest.Select(p.Guests, episodeNumber)
			if len(p.Guests) > 0 && episodeNumber == 0 {
			...
			runner := &pipeline.Runner{
				...
				ProgramSummarizer: programsummary.NewLLMProgramSummarizer(llmClient, prompts["summary"], stepTemp(cfg.LLM, "summary"), p.Program.EffectiveSummaryLength()),
