...
func newEpisodegenCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Long: fmt.Sprintf(`collect → rundown → script → synth → assemble → manifest を一括実行します。
...
例:
  ...
  vox-radio --config /path/to/vox-radio.yaml episodegen --spec episode-spec.yaml`, referenceURL("README.md")),
