...
| ジャンル別設定 (profile) | `sample-profiles/<genre>/profile.yaml` | program / corners（各コーナーの source でデータソース指定） / assets |
...
プロファイルのサンプルは `sample-profiles/` ディレクトリに用意しています。`sample-profiles/tech/`（技術ニュース用）と `sample-profiles/test/`（動作確認用）です。これらをコピー・編集して利用してください。詳細は [sample-profiles/README.md](sample-profiles/README.md) を参照してください。
...
vox-radio script --in work/articles.json --out work/script.json \
    --profile sample-profiles/tech/profile.yaml
