...
**トレードオフ**: マニフェスト・キャッシュのスキーマが増える。回数はキャッシュ有効が前提。

> 注: 本 ADR 採用当時はキャッシュ有効化を `cache.enabled` かつ `program.id` 設定で制御しており、無効時は回番号も不明でゲストが出演しないため影響はないとしていた。ADR-0043 で `cache.enabled` を廃止し `program.id` を必須化、キャッシュを常時有効としたため、現在は回数記録が常に有効となる。
