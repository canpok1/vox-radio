...
- OP/ED ジングルはコードが `program.opening_jingle`/`ending_jingle` に基づきスクリプト先頭/末尾へ注入。中間アイキャッチは LLM が `insertions` の `type=jingle` で配置。
...
breaking change あり：既存 `script.json` の `se_name` キーを `asset_name` に移行要。プロファイルの jingle/bgm 設定構造は変わらないが、`program.opening_jingle`/`ending_jingle` によるキー指定は引き続き使用可能。
